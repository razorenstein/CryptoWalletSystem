...
describe('WalletService', () => {
  ...
  describe('createWallet', () => {
    it('should create a new wallet with a generated UUID if the user does not exceed the limit', async () => {
      userWalletsFileManagementService.getUserWalletIds = jest
        .fn()
        .mockResolvedValue([]); // No wallets yet
      walletFileManagementService.getWallet = jest.fn().mockResolvedValue(null); // Wallet doesn't exist
      walletFileManagementService.saveWallet = jest
        .fn()
        .mockResolvedValue(undefined);
      userWalletsFileManagementService.addUserWallet = jest
        .fn()
        .mockResolvedValue(undefined);
      ...
      expect(
        userWalletsFileManagementService.addUserWallet,
      ).toHaveBeenCalledWith('user1', mockUUID);
    ...
    it('should throw MaxWalletsExceededException if user exceeds the max wallets', async () => {
      userWalletsFileManagementService.getUserWalletIds = jest
        .fn()
        .mockResolvedValue(['wallet1']); // Already 1 wallet
  ...
  describe('getWallet', () => {
    it('should retrieve wallet if user owns the wallet', async () => {
      ...
      walletFileManagementService.getWallet = jest
        .fn()
        .mockResolvedValue(mockWallet);

      const result = await walletService.getWallet('user1', 'wallet1');

      expect(walletFileManagementService.getWallet).toHaveBeenCalledWith(
        'wallet1',
      );
  ...
  describe('deleteWallet', () => {
    it('should delete a wallet if the user owns it', async () => {
      ...
      walletFileManagementService.getWallet = jest
        .fn()
        .mockResolvedValue(mockWallet);
      walletFileManagementService.deleteWallet = jest
        .fn()
        .mockResolvedValue(undefined);
      userWalletsFileManagementService.removeUserWallet = jest
        .fn()
        .mockResolvedValue(undefined);

      await walletService.deleteWallet('user1', 'wallet1');

      expect(walletFileManagementService.deleteWallet).toHaveBeenCalledWith(
        'user1',
        'wallet1',
      );
      expect(
        userWalletsFileManagementService.removeUserWallet,
      ).toHaveBeenCalledWith('user1', 'wallet1');
    });
  });
  ...
  describe('calculateTotalValue', () => {
    it('should calculate the total value of the wallet in USD', async () => {
      ...
      walletFileManagementService.getWallet = jest
        .fn()
        .mockResolvedValue(mockWallet);
      rateService.getAssetRates = jest
        .fn()
        .mockResolvedValue(mockRatesResponse);

      const result = await walletService.calculateTotalValue(
        'user1',
        'wallet1',
        'USD',
      );
  ...
  describe('addAsset', () => {
    ...
    it('should update an existing asset in the wallet', async () => {
      ...
      const bitcoin = result.cryptoAssets.find(
        (asset) => asset.id === 'bitcoin',
      );
    ...
    it('should throw WalletNotFoundException if wallet is not found', async () => {
      ...
      await expect(
        walletService.addAsset('user1', 'walletNotExist', {
          assetId: 'bitcoin',
          amount: 1,
        }),
  ...
  describe('removeAsset', () => {
    ...
    it('should throw InsufficientAssetAmountException when trying to remove more than available', async () => {
      ...
      await expect(
        walletService.removeAsset('user1', 'wallet1', removeAssetDto),
      ).rejects.toThrow(InsufficientAssetAmountException);
    });
    ...
    it('should throw AssetNotFoundException if asset is not found', async () => {
      ...
      await expect(
        walletService.removeAsset('user1', 'wallet1', {
          assetId: 'dogecoin',
          amount: 1,
        }),
