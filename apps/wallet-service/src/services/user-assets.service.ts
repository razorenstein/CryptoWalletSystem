...
export class UserAssetsService {
  ...
  async calculateTotalUserAssetsValue(
    ...
    this.logger.log(
      `Calculating total user assets value`,
      UserAssetsService.name,
      { userId, currency },
    );
    ...
    for (const walletId of walletIds) {
      const walletDetail = await this.walletService.calculateTotalValue(
        userId,
        walletId,
        currency,
      );
    ...
    this.logger.log(
      `Total user assets value calculated`,
      UserAssetsService.name,
      { userId, currency, totalValue },
    );
