...
export class WalletService {
  ...
  async addAsset(userId: string, walletId: string, addAssetDto: AddAssetDto): Promise<Wallet> {
    ...
    if (!wallet) {
        this.logger.warn(`Wallet not found for adding asset`, WalletService.name, { userId, walletId });
        throw new WalletNotFoundException(walletId, userId);
    }
    ...
    if (assetIndex !== -1) {
        ...
        this.logger.log(`Updated asset in wallet`, WalletService.name, { userId, walletId, assetId: addAssetDto.assetId, value: wallet.cryptoAssets[assetIndex].amount });
    } else {
        ...
        this.logger.log(`Added new asset to wallet`, WalletService.name, { userId, walletId, assetId: newAsset.id, value: newAsset.amount });
    }
  ...
  async removeAsset(userId: string, walletId: string, removeAssetDto: RemoveAssetDto): Promise<Wallet> {
    ...
    await this.walletFileManagementService.saveWallet(wallet);
    this.logger.log(`Asset ${removeAssetDto.assetId} updated in wallet ${walletId} for user ${userId}`, WalletService.name);
  
    return wallet;
  }
  
  async rebalance(userId: string, walletId: string, targetPercentages: Record<string, number>): Promise<void> {
    this.logger.log(`Rebalancing wallet`, WalletService.name, { userId, walletId, targetPercentages });
  
    // Calculate the current total value of the wallet
    const { wallet, totalValue, currency } = await this.calculateTotalValue(userId, walletId, 'USD');
    ...
    for (const asset of wallet.cryptoAssets) {
      ...
      if (targetAmount > asset.amount) {
        ...
        await this.addAsset(userId, walletId, { assetId: asset.id, amount: addAmount });
      } else if (targetAmount < asset.amount) {
        const removeAmount = asset.amount - targetAmount;
        await this.removeAsset(userId, walletId, { assetId: asset.id, amount: removeAmount });
      }
    }
  
    this.logger.log(`Rebalance complete for wallet ${walletId}`, WalletService.name);
  }
