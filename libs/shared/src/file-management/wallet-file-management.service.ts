...
export class WalletFileManagementService {
  ...
  private createDirectoryIfNotExists(directoryPath: string, context: string) {
    try {
      if (!fs.existsSync(directoryPath)) {
        ...
        this.logger.log(`Created directory`, WalletFileManagementService.name, {
          directoryPath,
          context,
        });
      }
      ...
      this.logger.error(
        ...
        error.stack,
        WalletFileManagementService.name,
  ...
  async saveWallet(wallet: Wallet): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`Saved wallet`, WalletFileManagementService.name, {
        walletId: wallet.id,
      ...
      throw new InternalServerErrorException(
        `Failed to save wallet: ${wallet.id}`,
      );
    }
  }
  ...
  async getWallet(walletId: string): Promise<Wallet | null> {
    ...
    try {
      if (fs.existsSync(filePath)) {
        ...
        this.logger.log(`Retrieved wallet`, WalletFileManagementService.name, {
          walletId,
        });
        return JSON.parse(walletData);
      } else {
        this.logger.warn(`Wallet not found`, WalletFileManagementService.name, {
          walletId,
        });
        return null;
      }
      ...
      throw new InternalServerErrorException(
        `Failed to retrieve wallet: ${walletId}`,
      );
    }
  }
  ...
  async deleteWallet(userId: string, walletId: string): Promise<void> {
    ...
    try {
      if (!fs.existsSync(filePath)) {
        this.logger.warn(
          `Attempted to delete non-existing wallet`,
          WalletFileManagementService.name,
          { walletId },
        );
      ...
      this.logger.log(`Deleted wallet`, WalletFileManagementService.name, {
        walletId,
      });
    } catch (error) {
      this.logger.error(
