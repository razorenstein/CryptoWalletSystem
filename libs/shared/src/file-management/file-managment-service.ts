...
export class FileManagementService {
  ...
  async saveToFile<T>(fileName: string, data: T): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`Saved data to file`, FileManagementService.name, {
        fileName,
      });
    } catch (error) {
      this.logger.error(
        ...
        error.stack,
        FileManagementService.name,
        { fileName },
      );
      throw new InternalServerErrorException(
        `Failed to save data: ${fileName}`,
      );
    }
  }
  ...
  async readFromFile<T>(fileName: string): Promise<T | null> {
    ...
    try {
      if (fs.existsSync(filePath)) {
        ...
        this.logger.log(`Read data from file`, FileManagementService.name, {
          fileName,
        });
        return JSON.parse(data) as T;
      } else {
        this.logger.warn(`File not found`, FileManagementService.name, {
          fileName,
        });
        return null;
      }
